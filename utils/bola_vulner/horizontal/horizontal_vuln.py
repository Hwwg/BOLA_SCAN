...
if _judge_votes < 1:
    _judge_votes = 1
try:
    _judge_vote_temperature = float(os.getenv('BOLASCAN_JUDGE_VOTE_TEMPERATURE', '0'))
except Exception:
    _judge_vote_temperature = 0.0
    ...
    def bola_vul_judgement(self, execution_results):
        ...
        def _build_structured_bola_evidence(
            ...
            if case_type == "container_boundary":
                ...
                # C is the substituted value in the test request. The matching
                # identifier in the control request is the E-side comparison;
                # it need not appear among the test request's alias sources.
                # Do not mistake an unrelated fixed D-side resource for it.
                preferred_location = tested_alias_position or test_meta.get("target_position")

                def boundary_identifier(request, execution, step):
                    from urllib.parse import urlsplit, unquote
                    request = request if isinstance(request, dict) else {}
                    locations = [preferred_location] if preferred_location else ["path", "query", "body", "header"]
                    for location in locations:
                        if location == "path":
                            route_parts = str((step or {}).get("route", "")).strip("/").split("/")
                            marker = "{" + current_param_name + "}"
                            if marker not in route_parts:
                                continue
                            path_values = (execution or {}).get("request_data", {})
                            if isinstance(path_values, dict) and current_param_name in path_values:
                                return path_values[current_param_name], location
                            url_parts = urlsplit(str(request.get("url", ""))).path.strip("/").split("/")
                            if len(route_parts) == len(url_parts):
                                return unquote(url_parts[route_parts.index(marker)]), location
                        else:
                            fields = {
                                "query": request.get("params"),
                                "body": request.get("json") or request.get("data"),
                                "header": request.get("headers"),
                            }.get(location)
                            if isinstance(fields, dict):
                                field_path = find_matching_path(fields, [current_param_name])
                                if field_path:
                                    return nested_get(fields, field_path), location
                    return None, preferred_location

                container_b_value, _ = boundary_identifier(data_request_params, data_exec, data_step)
                # Native single-location boundary cases may have no alias
                # metadata or target_position. Recover C from the actual probe.
                if container_a_value is None:
                    container_a_value, tested_alias_position = boundary_identifier(test_request_params, test_exec, test_step)
                    attacker_value = container_a_value
                    injected_value = container_a_value
                    tested_alias_value = container_a_value
                    tested_alias_source = "C"
                # Same-account probing has no other-account victim reference.
                victim_value = None
            ...
            if case_type in ("multi_param", "container_boundary") and tested_alias_position not in (None, "", [], {}):
        ...
        def _compare_response_params(current_param_name,data_params, test_params, data_step, test_step, data_request_params, test_request_params, type_tag=None, evidence_data=None, test_meta=None):
            ...
            if not _llm_disabled:
                ...
                # 采样若干次（每次采样内部仍有有限重试兜底），使用配置的投票温度（默认 0）
